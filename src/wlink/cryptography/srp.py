...
from wlink.utility.string import int_to_bytes, bytes_to_int
...
class WoWSrp:
    ...
    def compute_proof(
        self, session_key: int, salt: int, server_public: int, client_public: int
    ) -> Tuple[int, int]:
...
class WoWSrpServer(WoWSrp):
    def __init__(
        ...
        prime: Optional[int] = None,
        generator: int = 7,
        salt: int = random.getrandbits(1024),
        server_private: int = random.getrandbits(1024),
    ):
        ...
        self.server_public = (
            (self.multiplier * self.password_verifier)
            + pow(self.generator, server_private, self.prime)
        ) % self.prime
    ...
    def process(self, session_proof: int, client_public: int):
        ...
        self._server_premaster_secret = pow(
            (
                client_public
                * pow(self.password_verifier, self.common_secret, self.prime)
            ),
            self.server_private,
            self.prime,
        )
        ...
        self.session_proof, self.session_proof_hash = self.compute_proof(
            bytes_to_int(self.session_key), self.salt, self.server_public, client_public
        )
...
class WoWSrpClient(WoWSrp):
    def __init__(
        ...
        password = None
        self.client_private = (
            random.getrandbits(1024) if client_private is None else client_private
        )
    ...
    def process(self, server_public: int, salt: int) -> Tuple[int, int]:
        ...
        self.client_premaster_secret = pow(
            ...
            (self.client_private + (self.common_secret * self.password_hash)),
            self.prime,
        )
...
def generate_crc(
    client_public,
    crc_salt: int,
    game_files_root: str = "C:\\Users\\dinne\\Downloads\\World of Warcraft 3.3.5a (no install)\\",
):
    filenames = {
