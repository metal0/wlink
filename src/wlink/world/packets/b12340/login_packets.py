...
def make_SMSG_LOGIN_VERIFY_WORLD(map: int, position, rotation) -> bytes:
    return SMSG_LOGIN_VERIFY_WORLD.build(
        dict(map=map, position=position, rotation=rotation)
    )
...
def make_SMSG_LOGOUT_RESPONSE(reason: int, instant_logout: bool) -> bytes:
    return SMSG_LOGOUT_RESPONSE.build(
        dict(
            reason=reason,
...
CMSG_PLAYER_LOGIN = construct.Struct(
    "header" / ClientHeader(Opcode.CMSG_PLAYER_LOGIN, 8), "guid" / GuidConstruct(Guid)
)


def make_CMSG_PLAYER_LOGIN(guid):
    return CMSG_PLAYER_LOGIN.build(dict(guid=guid))
...
__all__ = [
    "make_CMSG_LOGOUT_CANCEL",
    "make_CMSG_LOGOUT_REQUEST",
    "make_SMSG_LOGOUT_RESPONSE",
    "make_SMSG_LOGOUT_COMPLETE",
    "make_SMSG_LOGIN_VERIFY_WORLD",
    "make_CMSG_PLAYER_LOGIN",
    "make_SMSG_LOGOUT_CANCEL_ACK",
    "CMSG_LOGOUT_CANCEL",
    "CMSG_LOGOUT_REQUEST",
    "CMSG_PLAYER_LOGIN",
