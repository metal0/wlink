...
def make_CMSG_AUCTION_SELL_ITEM(auctioneer, auction_items: list, bid: int, buyout: int, expiry_time: int):
	if type(auctioneer) is int:
		auctioneer = Guid(value=auctioneer)

	data_size = 8 + len(auction_items) * (8 + 4) + 4 + 4 + 4 + 4
	return CMSG_AUCTION_SELL_ITEM.build(dict(
...
def make_CMSG_AUCTION_PLACE_BID(auctioneer, auction_id: int, price: int):
	...
	data_size = 8 + 4 + 4
	return CMSG_AUCTION_PLACE_BID.build(dict(
		header=dict(size=4 + data_size),
...
def make_CMSG_AUCTION_LIST_ITEMS(
	auctioneer, search_term: str, list_start: int = 0, min_level=0,
	max_level=80,
	...
	return CMSG_AUCTION_LIST_ITEMS.build(dict(
		header=dict(size=4 + data_size),
		auctioneer=auctioneer,
...
def make_CMSG_AUCTION_LIST_OWNER_ITEMS(auctioneer, list_start: int = 0):
	...
	return CMSG_AUCTION_LIST_OWNER_ITEMS.build(dict(
		header=dict(size=4 + data_size),
		auctioneer=auctioneer,
...
def make_CMSG_AUCTION_LIST_PENDING_SALES(auctioneer):
	...
	return  CMSG_AUCTION_LIST_PENDING_SALES.build(dict(
		header=dict(size=4 + data_size),
		auctioneer=auctioneer,
	))
...
__all__ = [
	'SMSG_AUCTION_LIST_PENDING_SALES', 'PendingSale', 'CMSG_AUCTION_LIST_PENDING_SALES',
	'make_CMSG_AUCTION_LIST_PENDING_SALES', 'make_CMSG_AUCTION_LIST_ITEMS', 'make_CMSG_AUCTION_LIST_OWNER_ITEMS',
	'make_CMSG_AUCTION_PLACE_BID', 'make_CMSG_AUCTION_SELL_ITEM', 'CMSG_AUCTION_PLACE_BID', 'CMSG_AUCTION_SELL_ITEM',
	'CMSG_AUCTION_LIST_ITEMS', 'CMSG_AUCTION_HELLO', 'CMSG_AUCTION_LIST_OWNER_ITEMS', 'CMSG_AUCTION_REMOVE_ITEM',
	'SMSG_AUCTION_HELLO', 'SMSG_AUCTION_LIST_RESULT', 'SMSG_AUCTION_COMMAND_RESULT', 'AuctionInfo',
	'AuctionEnchantmentInfo', 'AuctionSellData', 'MSG_AUCTION_HELLO',
]