...
class ServerSize(Construct):
    ...
    def _build(self, obj: int, stream, context, path):
        ...
        if obj < 0:
            raise IntegerError(
                f"StreamSize cannot build from negative number {obj}", path=path
            )
        result = bytearray()
...
def ClientHeader(opcode=None, body_size=0):
    ...
    if opcode is None:
        ...
        opcode_con = construct.Default(
            construct.Const(opcode, PackEnum(Opcode, construct.Int32ul)), opcode
        )
...
def ServerHeader(opcode=None, body_size=0, is_large=False):
    if opcode is None:
        ...
        opcode_con = construct.ByteSwapped(
            construct.Default(
                construct.Const(opcode, PackEnum(Opcode, construct.Short)), opcode
            )
        )
...
__all__ = ["ServerHeader", "ServerSize", "ClientHeader", "is_large_server_packet"]
