...
def make_SMSG_ADDON_INFO(addon_data: bytes):
	...
	return SMSG_ADDON_INFO.build(dict(
		header=dict(opcode=Opcode.SMSG_ADDON_INFO, size=(3 if is_large else 2) + len(addon_data)),
...
def make_SMSG_CLIENTCACHE_VERSION(version: int = 3):
	return SMSG_CLIENTCACHE_VERSION.build(dict(
		version=version
	))
...
def make_SMSG_TRIGGER_CINEMATIC(cinematic_id: int) -> bytes:
	return SMSG_TRIGGER_CINEMATIC.build(dict(
...
def make_SMSG_TRIGGER_MOVIE(movid_id: int) -> bytes:
	return SMSG_TRIGGER_MOVIE.build(dict(
		header=dict(opcode=Opcode.SMSG_TRIGGER_MOVIE, size=4),
		movid_id=movid_id
	))
...
__all__ = [
	'SMSG_CRITERIA_UPDATE', 'SMSG_UPDATE_WORLD_STATES', 'SMSG_INIT_WORLD_STATES', 'SMSG_ADDON_INFO',
	'SMSG_TRIGGER_CINEMATIC', 'SMSG_TRIGGER_MOVIE', 'SMSG_CLIENTCACHE_VERSION', 'make_SMSG_TRIGGER_MOVIE',
	'make_SMSG_ADDON_INFO', 'make_SMSG_CLIENTCACHE_VERSION', 'make_SMSG_TRIGGER_CINEMATIC',
]