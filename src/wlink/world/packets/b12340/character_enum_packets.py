...
SMSG_CHAR_ENUM = construct.Struct(
    "header"
    / ServerHeader(
        Opcode.SMSG_CHAR_ENUM, 4 + construct.len_(construct.this.characters)
    ),
...
def make_CMSG_CHAR_RENAME(new_name: str, guid):
    ...
    :return: None.
    """
    return CMSG_CHAR_RENAME.build(
        dict(header=dict(size=len(new_name) + 1 + 8 + 4), guid=guid, new_name=new_name)
    )


RenameInfo = construct.Struct(
    "guid" / GuidConstruct(Guid), "new_name" / construct.CString("utf8")
)
...
SMSG_CHAR_RENAME = construct.Struct(
    ...
    / construct.IfThenElse(
        (
            construct.this.response == CharacterNameResponse.no_name
            or construct.this.response == CharacterNameResponse.reserved
        ),  # also when == CHAR_CREATE_ERROR
        construct.Pass,
        RenameInfo,
    ),
)
