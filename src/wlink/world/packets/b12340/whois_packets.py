...
def make_SMSG_WHOIS(name: str):
	...
	return SMSG_WHOIS.build(dict(
		header=dict(size=(3 if is_large else 2) + len(name) + 1),
		name=name
	))

# WHO is a non-privileged ingame character query
CMSG_WHO = construct.Struct(
	'header' / ClientHeader(Opcode.CMSG_WHO, body_size=4 + 4 + 0 + 1 + 4 + 4 + 4 + 4 * 0 + 4 + (0 + 1) * 0),
...
def make_CMSG_WHO(
	name: str = '', guild_name: str = '',
	race=None, combat_class=None,
...
def make_SMSG_WHO(matches: list):
	...
	return SMSG_WHO.build(dict(
		header=dict(size=(3 if is_large else 2) + len(matches) + 1),
		matches=matches
	))

__all__ = [
	'SMSG_WHO', 'SMSG_WHOIS', 'CMSG_WHOIS', 'CMSG_WHO', 'WhoMatchInfo', 'make_CMSG_WHO', 'make_CMSG_WHOIS',
	'make_SMSG_WHOIS',
]