...
def make_CMSG_AUTH_SESSION(
    ...
    return CMSG_AUTH_SESSION.build(
        dict(
            build=build,
...
def make_SMSG_AUTH_CHALLENGE(
    ...
    return SMSG_AUTH_CHALLENGE.build(
        dict(
...
SMSG_AUTH_RESPONSE = construct.Struct(
    ...
    "billing" / BillingInfo,  #
    "expansion"
    / construct.Default(PackEnum(Expansion), Expansion.wotlk),  # which WoW expansion
    "queue_position"
    / construct.Default(
        construct.Switch(
            construct.this.response == AuthResponse.wait_queue,
            {True: construct.Int32ul, False: construct.Pass},
        ),
        construct.Pass,
    ),
)
...
def make_SMSG_AUTH_RESPONSE(
    ...
    return SMSG_AUTH_RESPONSE.build(
        dict(
