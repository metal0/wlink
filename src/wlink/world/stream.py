...
class WorldClientStream(WorldStream):
    ...
    async def next_decrypted_packet(self):
        ...
        try:
            ...
            if is_large:
                header_data += self.protocol.crypto.decrypt(
                    await self._receive_some(max_bytes=1)
                )
            ...
            try:
                ...
                fragmentation = abs(
                    int(100 * (num_packets / self._average_num_frames - 1))
                )

                logger.log(
                    "PACKETS",
                    f"fragmentation: {fragmentation}%, average: {self._average_num_frames}",
                )
                logger.log("PACKETS", f"{data=}")
...
class WorldServerStream(WorldStream):
    ...
    def encrypt_packet(self, data: bytes, header):
        packed_header = ServerHeader().build(
            dict(opcode=header.opcode, size=header.size)
        )
